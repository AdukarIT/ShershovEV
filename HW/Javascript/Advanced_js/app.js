...
let californiaFilter = data.filter(function(item) {
    return item.state == 'California';
});
console.log(californiaFilter);

let populationFilter = californiaFilter.filter(function(item) {
    return parseFloat(item.growth_from_2000_to_2013) > 0;
});
...
let south = data.filter(function(item) {
    return item.latitude >= 25 && item.latitude <= 30;
});
console.log(south);

let southPeople = south.reduce(function(acc, curr) {
    return acc + parseInt(curr.population);
}, 0);
console.log(Math.ceil(southPeople / 1000000) + ' million');
...
// let dCities = data.filter(function(item) {
//     return item.city.substr(0, 1) === "D";
// });
// console.log(dCities);
